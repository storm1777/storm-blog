...
(def fixtures
  [{:db/id -1
   ...
   {:db/id -14
    ...
    :widget/owner -1
    :widget/order 6
    :widget/content "The park is divided into two sections: the Upper Lakes and the Lower Lakes. On day one, we bought our tickets and were ferryed onto a boat that took us over to the Upper Lakes. They suggested that we walk up until we got to their station, and then catch the bus back down."}
   {:db/id -15
    :widget/type :par
    :widget/owner -1
    :widget/order 7
   ...
   {:db/id -16
    ...
    :widget/owner -1
    :widget/order 8
    :widget/content "In general, the Upper Lakes are less busy than the Lower Lakes, and have just as much to offer. If you’re a serious photographer of any kind, you’ll likely have better luck in this section finding the space and time to set up a human-free shot."}
   {:db/id -17
    :widget/type :par
    :widget/owner -1
    :widget/order 9
   ...
   {:db/id -22
    ...
    :widget/owner -1
    :widget/order 14
    :widget/content "Lodging is available in the area, of course, due to the draw of the park--but options are limited. There are a few hotels in park, a few outside, and several hostels in the surrounding area. We went with a private room in an Airbnb house. The top floor of the home was dedicated to Airbnb--we had a private room and bath, and a kitchen that we were sharing with one other family (though we never saw them, and never had an issue with room in the kitchen)."}
   {:db/id -23
    :widget/type :par
    :widget/owner -1
    :widget/order 14
   ...
   {:db/id -28
    :widget/type :section
    :widget/owner -6
    :widget/content "Section example"}
   {:db/id -29
    :widget/type :par
    :widget/owner -6
    :widget/order 1
